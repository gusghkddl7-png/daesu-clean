...
function DateField({
  ...
  return (
    <div className={`md:col-span-4 ${className}`}>
      <label className="text-xs font-medium text-gray-600 mb-1 block">연월일</label>
      <div className="border rounded h-10 px-3 flex items-center justify-between cursor-pointer" onClick={openPicker}>
        <input
          ref={ref}
          type="date"
          name="availableDate"
          className="h-9 text-sm outline-none w-full bg-transparent cursor-pointer"
          value={value}
      ...
      <div className="mt-2 flex items-center gap-4 text-sm">
        <label className="inline-flex items-center gap-2">
          <input type="checkbox" checked={negotiable} onChange={(e) => onNegotiable(e.target.checked)} />
          협의가능
        </label>
        <label className="inline-flex items-center gap-2">
          <input type="checkbox" checked={vacant} onChange={(e) => onVacant(e.target.checked)} />
          공실
        </label>
      </div>
    </div>
...
export default function NewListingPage() {
  ...
  useEffect(() => {
    ...
    /* tabflow: disabled */ // /* tabflow: disabled */ // document.addEventListener("keydown", killer, false);
...
const prefix = useMemo(() => getPrefix(f.dealType, f.buildingType), [f.dealType, f.buildingType]);
  ...
  <h1 className="text-xl font-semibold text-center">매물 등록</h1>
  <div />
</div>
