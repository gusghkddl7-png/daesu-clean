...
function RelocateAdvSearch() {
  useEffect(() => {
    try {
      const findByText = (txts) => {
        const all = Array.from(document.querySelectorAll("body *"));
        const norm = (s) => (s ?? "").replace(/\s/g, "");
        for (const el of all) {
          const t = norm(el.textContent);
          for (const x of txts) if (t.includes(norm(x))) return el;
        }
        return null;
      };

      const label = findByText(["(검색 시 표시)", "(검색시표시)", "검색 시 표시", "검색시 표시"]);
      const orig  = Array.from(document.querySelectorAll("button, a"))
        .find((el) => (el as HTMLElement).innerText?.trim() === "상세검색");

      if (!label || !orig) return;

      // 이미 내려놨으면 스킵
      if (document.getElementById("relocated-adv")) return;
      ...
      // 상단 원본은 숨김
      (orig as HTMLElement).style.display = "none";
    } catch {}
  }, []);
  return null;
}
...
function UrgentInquiries() {
  // TODO: 실제 데이터로 교체하세요. (예: DB/스프레드시트 연동)
  const data: UrgentInquiry[] = useMemo(() => [
    { id: "U-001", name: "홍길동", phone: "010-1234-5678", area: "천호/길동", deadline: new Date(Date.now()+29*86400000).toISOString().slice(0,10), budget: "월 100/50", type: "원룸", note: "반려견 O" },
    { id: "U-002", name: "김영희", phone: "010-2222-3333", area: "성내",       deadline: new Date(Date.now()+19*86400000).toISOString().slice(0,10), budget: "전세 2.5억", type: "투룸" },
    { id: "U-003", name: "박철수", phone: "010-4444-5555", area: "둔촌",       deadline: new Date(Date.now()+9*86400000 ).toISOString().slice(0,10), budget: "월 70/70",  type: "오피스텔", note: "엘베 필수" },
  ], []);

  const items = data.map(it => ({ ...it, d: _daysLeft(it.deadline) }))
                    .filter(x => x.d >= 0)   // 마감 지난 건 숨김
                    .sort((a, b) => a.d - b.d);
