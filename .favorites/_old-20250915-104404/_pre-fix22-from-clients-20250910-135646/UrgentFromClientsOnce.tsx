...
function removeBadge(){
  const b=document.getElementById("urgent-bridge-badge"); if(b) b.remove();
}
...
export default function UrgentFromClientsOnce(){
  useEffect(()=>{
    // /clients 에서만 동작
    if ((location.pathname.split("?")[0]||"") !== "/clients") return;
    ...
    const run=()=>{
      ...
      if(!table){ if(tries<20){ ensureBadge(`표 대기중(${tries})…`); setTimeout(run,500);} return; }
      ...
      if(!tbody){ if(tries<20){ ensureBadge(`행 대기중(${tries})…`); setTimeout(run,500);} return; }
    ...
    return ()=>{ stopped=true; mo.disconnect(); clearTimeout(stopTimer); removeBadge(); };
  },[]);
  return null;
}