...
export default function NewListingPage() {
  // === CODE SYNC (header <-> hidden) ===
  useEffect(()=>{
    if (typeof window === "undefined") return;
    const header = document.querySelector('input[name="code"]:not([type="hidden"])') as HTMLInputElement | null;
    const hidden = document.querySelector('input[name="code"][type="hidden"]') as HTMLInputElement | null;
    const sync = () => { if (header && hidden) hidden.value = header.value; };
    sync();
    header?.addEventListener("input", sync);
    header?.addEventListener("change", sync);
    return ()=>{ header?.removeEventListener("input", sync); header?.removeEventListener("change", sync); };
  }, []);
  // === PREFILL FROM SESSION START ===
  useEffect(()=>{
    if (typeof window === "undefined") return;
    try{
      ...
      const apply = ()=>{
        for (const [k,v] of Object.entries(rec as any)){
  ...
  // === PREFILL FROM SESSION END ===
  // === CODE AUTOGEN START (safe injection) ===
  const codePrefixOf = (deal:string, bt:string) => {
    if ((bt||"").includes("상가") || (bt||"").includes("사무실")) return "R";
    if ((bt||"").includes("재개발") || (bt||"").includes("재건축")) return "J";
    if (deal==="월세") return "BO";
    if (deal==="전세") return "BL";
    if (deal==="매매") return "BM";
    return "BO";
  };
  const nextCodeFor = (prefix:string) => {
    try {
      const key = "codeSeq:"+prefix;
      let n = Number(localStorage.getItem(key)||"0"); if(!isFinite(n)) n=0;
  ...
  useEffect(()=>{
    ...
    const codeEl  = document.querySelector('input[name="code"]') as HTMLInputElement | null;
    const dealEl  = document.querySelector('[name="dealType"]') as HTMLInputElement | HTMLSelectElement | null;
    const btEl    = document.querySelector('[name="buildingType"]') as HTMLInputElement | HTMLSelectElement | null;

    if (!codeEl) return;

    // 잠금 기본값
    codeEl.readOnly = true;
    (codeEl as any).dataset.locked = "1";

    const assign = () => {
      const deal = (dealEl as any)?.value ?? "";
      const bt   = (btEl   as any)?.value ?? "";
      const prefix = codePrefixOf(deal, bt);
      if (!codeEl.value || (codeEl as any).dataset.autofill === "1") {
        const next = nextCodeFor(prefix);
        codeEl.value = next;
        (codeEl as any).dataset.autofill = "1";
    ...
    // 최초 2회 시도(컨트롤드 폼 대비)
    assign(); setTimeout(assign, 120);

    const handler = () => {
      if (!codeEl || (codeEl as any).dataset.locked === "0") return; // 수정모드면 자동부여 X
      codeEl.value = "";
      (codeEl as any).dataset.autofill = "1";
      assign();
    };
    dealEl?.addEventListener("change", handler);
    btEl?.addEventListener("change", handler);
    return ()=>{ dealEl?.removeEventListener("change", handler); btEl?.removeEventListener("change", handler); };
  }, []);
  // === CODE AUTOGEN END ===
  ...
  <span className="flex items-center gap-2">
    <input name="code" readOnly data-locked="1" placeholder="코드번호" className="border rounded px-2 h-9 text-xs md:text-sm w-40 md:w-48" />
    <button type="button" className="border px-2 py-1 rounded text-[10px] md:text-xs" onClick={()=>{
...
<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        {/* 수정 전용: 거래완료 */}         <label className="inline-flex items-center gap-2 mt-2">           <input type="checkbox" name="completed" />           <span>거래완료</span>         </label>
        {/* 액션바 */}
