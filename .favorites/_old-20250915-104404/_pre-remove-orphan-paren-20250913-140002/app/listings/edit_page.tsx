  ...
  useEffect(() => {
    ...
    try {
      ...
      Object.entries(rec).forEach(([k, v]) => {
        const el = document.querySelector(`[name="${k}"]`) as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null;
        if (!el) return;
        const t = (el as HTMLInputElement).type;
        if (t === "checkbox") (el as HTMLInputElement).checked = !!v;
        else (el as HTMLInputElement).value = (v ?? "") as any;
      ...
      if (header && !header.value && rec.code) {
        header.value = rec.code;
...
<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        {/* 수정 전용: 거래완료 */}         <label className="inline-flex items-center gap-2 mb-2">           <input type="checkbox" name="completed" />           <span>거래완료</span>         </label>
        {/* 액션바 */}
