...
export default function NewListingPage() {
  ...
  const L = ({ children }: { children: any }) => <div className="text-xs font-medium text-gray-600 mb-1">{children}</div>;
        {/* 기본 / 거래 */}
        <Section title="기본 / 거래">
        ...
        {/* 액션바 */}         <div className="sticky bottom-0 z-10">           <div className="bg-white/90 backdrop-blur border rounded-xl px-4 py-3 flex items-center justify-between">             <div className="text-xs text-gray-500">               {hasError ? "필수/숫자/지번 형식을 확인해주세요." : "입력값이 유효합니다."}             </div>             <div className="flex gap-2">               <button type="button" className="border px-3 py-1.5 rounded" onClick={()=>history.back()}>취소</button>               <button type="submit" className="border px-3 py-1.5 rounded bg-blue-600 text-white">등록</button>             </div>           </div>         </div>         </form>     </main>   ); }
