  ...
  return (
    <main className="w-full max-w-none px-2 md:px-4 py-5">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button className="px-3 py-1.5 border rounded-lg bg-blue-600 text-white hover:opacity-90" onClick={()=>{ try{ const maxMap:any = {}; (Array.isArray(RAW)?RAW:[]).forEach((x:any)=>{ const m=String(x.code||"").match(/^([A-Za-z]+)-?(\d{1,})$/); if(m){ const p=m[1].toUpperCase(); const n=parseInt(m[2],10)||0; maxMap[p]=Math.max(maxMap[p]||0,n); } }); sessionStorage.setItem("codeMaxMap", JSON.stringify(maxMap)); }catch(e){}; router.push("/listings/new")}}>+ 매물등록</button>
      </div>
      ...
      <div className="border-y">
        <div className="overflow-auto">
          <table className="min-w-[1200px] w-full text-sm">
            ...
            <tbody>
              {rows.map(r => (
<tr key={r.id} onClick={()=>{ try{ sessionStorage.setItem("editRecord", JSON.stringify(r)); }catch(e){} router.push(`/listings/edit?id=${encodeURIComponent(r.id)}`) }} className={"border-t cursor-pointer hover:opacity-90 " + (r.completed ? "bg-black text-white/30" : (r.vacant ? "bg-pink-50" : ""))}>
