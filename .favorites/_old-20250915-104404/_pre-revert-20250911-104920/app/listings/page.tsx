...
import CompactifyListings from "../components/CompactifyListings";
import { UiScaleFloating } from "../components/UiScale";
...
/** 상단의 기존 "상세검색" 버튼을 숨기고,
 *  "(검색 시 표시)" 라벨 바로 뒤에 '상세검색' 버튼 복제/배치 */
/** 급한 임차 문의 섹션 (D-day 색상: 30=하늘, 20=주황, 10=빨강 / 마감 지나면 숨김) */




/** 상세검색을 상단바에서 숨기고, 검색 인풋(placeholder에 "검색" 포함)의 오른쪽에 배치 */
/** 급한 임차 문의 섹션 (마감 지난 항목은 자동 숨김) */




/** 상단바의 "상세검색"은 숨기고, 검색 인풋 오른쪽에 버튼을 배치.
 *  초기 렌더 타이밍/재렌더 대비를 위해 짧은 interval과 MutationObserver 사용. */




/** 급한 임차 문의 (가로 레이아웃 / 30·20·10일 색상 / 만료 숨김) */




/** 상세검색 버튼을 상단바에서 숨기고 검색창 오른쪽으로 이동 */
function RelocateAdvSearchV3() {
  useEffect(() => {
    let done = false;
    const moveOnce = () => {
      try {
        const allBtn = Array.from(document.querySelectorAll("button, a"));
        const advBtns = allBtn.filter(el => (el as HTMLElement).innerText?.trim() === "상세검색");
        const input =
          (document.querySelector('input[placeholder*="검색"]') as HTMLElement) ||
          (Array.from(document.querySelectorAll("input"))
            .find(el => (el as HTMLInputElement).placeholder?.includes("검색")) as HTMLElement);
        if (!input || advBtns.length === 0) return;

        if (!document.getElementById("relocated-adv")) {
          const clone = advBtns[0].cloneNode(true) as HTMLElement;
          clone.id = "relocated-adv";
          clone.style.display = "";
          clone.classList.remove("hidden");
          input.insertAdjacentElement("afterend", clone);
        }
        advBtns.forEach(b => ((b as HTMLElement).style.display = "none"));
        done = true;
      } catch {}
    };
    const iv = setInterval(() => { if (!done) moveOnce(); }, 200);
    setTimeout(() => clearInterval(iv), 2000);
    const mo = new MutationObserver(() => { if (!done) moveOnce(); });
...
function parseKDeadline(row: UrgentInquiry): Date | null {
  ...
  // 2) 숫자 필드 (month/day or month+phase)
  if (row.deadlineMonth) {
    const m = row.deadlineMonth;
    if (row.deadlineDay) return new Date(y, m - 1, row.deadlineDay);
  ...
  // 3) 한글 텍스트 "9월 5일" / "9월 초순|중순|하순"
  if (row.deadlineText) {
    const t = row.deadlineText.trim();
...
/** 고객문의 연동형 급한 임차 문의: 전체 리스트를 보여주되, 급한 항목(≤30일)을 맨 위로 */
...
type UrgentInquiry = {
  ...
  deadline: string; budget?: string; type?: string; note?: string;
};
...
function UrgentInquiriesV2() {
  // 임시 데이터(추후 DB/시트 연동)
  const data: UrgentInquiry[] = React.useMemo(() => [
    { id: "U-001", name: "홍길동", phone: "010-1234-5678", area: "천호/길동",
      deadline: new Date(Date.now()+29*86400000).toISOString().slice(0,10), budget: "월 100/50", type: "원룸", note: "반려견 O" },
    { id: "U-002", name: "김영희", phone: "010-2222-3333", area: "성내",
      deadline: new Date(Date.now()+19*86400000).toISOString().slice(0,10), budget: "전세 2.5억", type: "투룸" },
    { id: "U-003", name: "박철수", phone: "010-4444-5555", area: "둔촌",
      deadline: new Date(Date.now()+9*86400000 ).toISOString().slice(0,10), budget: "월 70/70",  type: "오피스텔", note: "엘베 필수" },
  ], []);

  // 글자 크기 (섹션 전체에 적용; 자식은 상대크기 사용)
  const [fontPx, setFontPx] = useState<number>(() => {
    if (typeof window === "undefined") return 14;
    const v = Number(localStorage.getItem("urgentFontPx") || "14");
    return Number.isFinite(v) ? v : 14;
  });
  useEffect(() => { try { localStorage.setItem("urgentFontPx", String(fontPx)); } catch {} }, [fontPx]);

  const items = data.map(it => ({ ...it, d: daysLeft(it.deadline) }))
                    .filter(x => x.d >= 0)
                    .sort((a,b) => a.d - b.d);
  if (items.length === 0) return null;

  return (
    <section className="mt-6" style={{ fontSize: `${fontPx}px`, lineHeight: 1.35 }}>
      {/* 헤더: 제목(좌) + 레전드/글자조절(우) */}
      <div className="flex items-center mb-2">
        <h2 className="font-semibold" style={{ fontSize: "1.15em" }}>급한 임차 문의</h2>

        <div className="ml-auto flex items-center gap-2" style={{ fontSize: "0.9em" }}>
      ...
      <div className="flex flex-col gap-2">
        {items.map(it => {
          ...
          return (
            <div key={it.id} className={`flex flex-wrap items-center gap-x-4 gap-y-1 p-3 rounded-xl border ${color}`}>
              <span className="font-medium px-2 py-1 rounded-md bg-white/70 border" style={{ fontSize: "0.85em" }}>{`D-${it.d}`}</span>
              ...
              <span className="ml-auto text-gray-600" style={{ fontSize: "0.95em" }}>{it.deadline}</span>
            </div>
          );
        })}
      </div>
    ...
    <div className="w-full max-w-none px-4 md:px-6 py-6 space-y-4">
      <div className="flex items-center gap-3">
        <button className="border px-3 py-2 rounded-lg" onClick={()=>history.back()}>← 뒤로가기</button>
              <h1 className="text-2xl font-bold absolute left-1/2 -translate-x-1/2 top-2 z-10 text-center pointer-events-none">매물관리</h1>
      ...
      <div className="flex items-center gap-4 text-sm">
        ...
        <table className="w-full text-sm table-fixed table-remark-last">
          <colgroup>
            {COLS.map(c => (
