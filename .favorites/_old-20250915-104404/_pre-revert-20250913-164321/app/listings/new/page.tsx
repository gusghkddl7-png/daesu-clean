...
useEffect(() => {
  ...
  const prefixFrom = (deal?: string, bt?: string) => {
    const d = String(deal||""); const b = String(bt||"");
    if (b.includes("아파트")) return "C";
    if (b.includes("상가") || b.includes("사무실")) return "R";
    if (b.includes("재개발") || b.includes("재건축")) return "J";
    if (d === "전세") return "BL";
    if (d === "매매") return "BM";
    return "BO"; // 월세·기타 주거형
  };

  const nextCandidateFor = (prefix: string) => {
    const keySeq = "codeSeq:"+prefix;
    const keySet = "codes:"+prefix;
    let n = Number(localStorage.getItem(keySeq) || "0");
    const used = new Set((localStorage.getItem(keySet)||"").split(",").filter(Boolean));
    const pad = (x:number)=> String(x).padStart(4,"0");
    do { n += 1; } while (used.has(prefix + "-" + pad(n)));
    return { n, code: prefix + "-" + pad(n) };
  };

  const assign = () => {
    const prefix = prefixFrom((dealEl as any)?.value ?? "", (btEl as any)?.value ?? "");
    if (!prefix) return;
    if (codeEl.readOnly && (!codeEl.value || codeEl.dataset.autofill === "1")) {
      const { code } = nextCandidateFor(prefix);
      codeEl.value = code;
  ...
  btEl?.addEventListener("change", assign);
  dealEl?.addEventListener("change", assign);

  const onSubmit = () => {
    const v = codeEl.value || "";
    const m = v.match(/^([A-Z]+)-(\d{4})$/);
    if (m) {
      const prefix = m[1]; const num = Number(m[2]);
      try{
        const keySeq = "codeSeq:"+prefix;
        const keySet = "codes:"+prefix;
        const cur = Number(localStorage.getItem(keySeq)||"0");
        if (num > cur) localStorage.setItem(keySeq, String(num));
        const list = (localStorage.getItem(keySet)||"").split(",").filter(Boolean);
        if (!list.includes(v)) { list.push(v); localStorage.setItem(keySet, list.join(",")); }
      }catch{}
    }
  };
  ...
  return () => {
    btEl?.removeEventListener("change", assign);
    dealEl?.removeEventListener("change", assign);
