      ...
      <div className="border-y">
        <div className="overflow-auto">
          <table className="min-w-[1200px] w-full text-sm">
            ...
            <tbody>
              {rows.map(r => (
<tr key={r.id} onClick={()=>{ try{ sessionStorage.setItem("editRecord", JSON.stringify(r)); }catch(e){} router.push(`/listings/edit?id=${encodeURIComponent(r.id)}`) }} className={"border-t cursor-pointer hover:opacity-90 " + (r.completed ? "bg-black text-white/30" : (r.vacant ? "bg-pink-50" : ""))}>{ try{ sessionStorage.setItem("editRecord", JSON.stringify(r)); }catch(e){} router.push(`/listings/edit?id=${encodeURIComponent(r.id)}`) }} className={"border-t cursor-pointer hover:opacity-90 " + (r.completed ? "bg-black text-white/30" : (r.vacant ? "bg-pink-50" : ""))}>{ try{ sessionStorage.setItem("editRecord", JSON.stringify(r)); }catch(e){} router.push(`/listings/edit?id=${encodeURIComponent(r.id)}`) }} className={"border-t cursor-pointer hover:bg-gray-50 " + (r.completed ? "opacity-50" : (r.vacant ? "bg-pink-50" : ""))}>{
