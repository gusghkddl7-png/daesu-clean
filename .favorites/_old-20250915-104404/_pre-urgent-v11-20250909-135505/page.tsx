...
function UrgentInquiries() {
  ...
  // 글자 크기 조절(로컬 저장)
  const [fontPx, setFontPx] = React.useState<number>(() => {
    if (typeof window === "undefined") return 14;
    const v = Number(localStorage.getItem("urgentFontPx") || "14");
    return Number.isFinite(v) ? v : 14;
  });
  React.useEffect(() => { try { localStorage.setItem("urgentFontPx", String(fontPx)); } catch {} }, [fontPx]);
  ...
  return (
    <section className="mt-6" style={{ fontSize: `${fontPx}px` }}>
      {/* 헤더: 제목(좌) + 레전드/글자조절(우) */}
      <div className="flex items-center mb-2">
        <h2 className="text-lg font-semibold">급한 임차 문의</h2>
        ...
        {items.map(it => {
          ...
          return (
            <div key={it.id} className={`flex flex-wrap items-center gap-x-4 gap-y-1 p-3 rounded-xl border ${color}`}>
              ...
              <span className="ml-auto text-sm text-gray-600">{it.deadline}</span>
            </div>
          );
        })}
      </div>
