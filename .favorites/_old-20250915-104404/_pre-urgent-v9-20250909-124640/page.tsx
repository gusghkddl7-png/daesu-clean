...
type UrgentInquiry = {
  _id?: string; name?: string; phone?: string; area?: string;
  budget?: string; type?: string; note?: string;
  // 마감 정보: ISO(YYYY-MM-DD) 이거나, '9월 5일'/'9월 중순' 등 한글 텍스트
  deadline?: string;       // ISO
  deadlineText?: string;   // "9월 5일" | "9월 초순" | "9월 중순" | "9월 하순"
  deadlineMonth?: number;  // 1~12
  deadlineDay?: number;    // 1~31
  deadlinePhase?: "초순" | "중순" | "하순";
};
...
function UrgentInquiriesConnected() {
  const [rows, setRows] = useState<UrgentInquiry[]>([]);
  ...
  const items = rows.map(r => {
    ...
    .sort((a,b) => {
      ...
      if (au !== bu) return bu - au;               // 급한 항목 먼저
      if (a.d !== null && b.d !== null) return a.d - b.d; // 남은 일수 적은 순
  ...
  return (
    <section className="mt-14">
      <h2 className="text-lg font-semibold mb-2">급한 임차 문의</h2>
      <div className="flex flex-col gap-2">
        {items.map((it:any) => {
          ...
          const dDate  = it.deadlineDate ? it.deadlineDate.toISOString().slice(0,10) : (it.deadlineText || it.deadline || "-");
          return (
            <div key={it._id ?? it.name} className={`flex flex-wrap items-center gap-x-4 gap-y-1 p-3 rounded-xl border ${color}`}>
              <span className="text-xs font-medium px-2 py-1 rounded-md bg-white/70 border">{dLabel}</span>
              ...
              <span className="ml-auto text-sm text-gray-600">{dDate}</span>
            </div>
          );
        })}
      </div>
