...
export default function UrgentPage() {
  ...
  useEffect(() => {
    const killBar = () => {
      try {
        const isUrgent = typeof location!=="undefined" && location.pathname.toLowerCase().startsWith("/urgent");
        if (!isUrgent) return;
        const bar = document.getElementById("uniform-topbar");
        ...
        Array.from(document.querySelectorAll("main h1")).forEach(el=>{
          ...
          if (t.includes("고객문의") || t.includes("임차문의") || t.includes("급한 임차문의"))
            (el as HTMLElement).style.display="none";
        });
      } catch {}
    };
    killBar();
    document.addEventListener("readystatechange", killBar);
    return () => document.removeEventListener("readystatechange", killBar);
  }, []);
