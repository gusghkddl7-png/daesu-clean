"use client";
function __hideUrgentHeader(){try{
  if (!(typeof location!=="undefined" && location.pathname.toLowerCase().startsWith("/urgent"))) return;
  Array.from(document.querySelectorAll("a,button"))
    .filter(el => ((el.textContent||"").trim()==="뒤로가기"))
    .forEach(el => (el as HTMLElement).style.display="none");
  Array.from(document.querySelectorAll("main h1")).forEach(el=>{
    const t=(el.textContent||"").trim();
    if (t.includes("고객문의")||t.includes("임차문의")||t.includes("급한 임차문의")) (el as HTMLElement).style.display="none";
  });
}catch{}}
if (typeof window!=="undefined"){setTimeout(__hideUrgentHeader,0);document.addEventListener("readystatechange",__hideUrgentHeader);}import MakeTablesCompact from "../components/MakeTablesCompact";
...
export default function UrgentPage() {
  ...
  return (
    <main id="urgent-root" className="min-h-screen p-2">
      <style id="__urgent_compact_css">#urgent-root .table-compact th,#urgent-root .table-compact td{padding-top:.20rem !important;padding-bottom:.20rem !important}</style>
      
      <MakeTablesCompact />

      <PageHeader title="급한 임차문의" right={
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2"><UiScaleInline /></div>
