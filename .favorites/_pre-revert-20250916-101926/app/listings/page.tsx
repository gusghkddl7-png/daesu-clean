...
const fmtWon = (v?: number) => (v !== undefined && v !== null) ? v.toLocaleString("ko-KR") : "-";
...
export default function ListingsPage() {
  ...
  const Select = (props: any) => <select {...props} className={"border rounded px-2 h-9 text-sm w-full " + (props.className||"")} />;  // === AutoCodeCountersSyncV2: 현재 RAW를 스캔해 접두사별 최대번호를 localStorage에 반영 ===
  useEffect(() => {
    try {
      const preBy = (deal: string, cat: string) => {
        if (/아파트/.test(cat)) return "C";
        if (/재개발|재건축/.test(cat)) return "J";
        if (/상가|사무/.test(cat)) return "R";
      ...
      (RAW || []).forEach((r) => {
        const cat = catOf(r.buildingType);
        const pre = preBy(r.dealType, cat);
        // 코드가 있으면 그 숫자를 최대값에 반영
        if (r.code) {
          const m = String(r.code).match(/^([A-Z]{1,3})-(\d{1,})$/);
          if (m && m[1] === pre) {
            const n = Number(m[2]);
            counters[pre] = Math.max(counters[pre] || 0, n);
            used.add(r.code);
          }
        }
      });
      // 기존 저장값과 병합(더 큰 쪽 유지)
      const old = JSON.parse(localStorage.getItem("codeCounters") || "{}");
      Object.keys(counters).forEach(k => { old[k] = Math.max(Number(old[k]||0), counters[k]); });
      localStorage.setItem("codeCounters", JSON.stringify(old));
      localStorage.setItem("usedCodes", JSON.stringify(Array.from(used)));
    } catch {}
  }, []);
  return (
        ...
        <div className="overflow-auto">
          <table className="min-w-[1200px] w-full text-sm">
            ...
            <tbody>
              {rows.map(r => (
<tr key={r.id}
    onClick={()=>{ try{ sessionStorage.setItem("editRecord", JSON.stringify(r)); }catch(e){} router.push(`/listings/edit?id=${encodeURIComponent(r.id)}`) }}
    className={"border-t cursor-pointer hover:opacity-90 " + (r.completed ? "bg-black text-white/30" : (r.vacant ? "bg-pink-50" : ""))}>
            ...
            <div className="p-5 overflow-auto space-y-5">
              ...
              <section className="rounded-xl border bg-gray-50 p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {BT_CATS.map(c => {
                    const active = btSel.includes(c.label);  // === AutoCodeCountersSyncV2: 현재 RAW를 스캔해 접두사별 최대번호를 localStorage에 반영 ===
  useEffect(() => {
    try {
      const preBy = (deal: string, cat: string) => {
        if (/아파트/.test(cat)) return "C";
