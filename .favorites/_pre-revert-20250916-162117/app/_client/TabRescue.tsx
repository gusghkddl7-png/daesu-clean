...
/** 전역 Tab 강제 안정화: /listings/new 에서만 활성화 (디버그 표식 포함) */
export default function TabRescue() {
  useEffect(() => {
    const isNewPage = () => location.pathname.startsWith("/listings/new");
    // 디버그 표식: <html data-tabrescue="on">
    try { document.documentElement.dataset.tabrescue = "on"; } catch {}
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      for (let i = 1; i <= xs.length; i++) {
        ...
        if (next && next !== from && isVis(next)) {
          try { (next as any).focus({ preventScroll: true }); } catch { next.focus(); }
          if (/^INPUT|TEXTAREA|SELECT$/.test(next.tagName)) {
            const n: any = next;
            try { if (n.select) n.select(); } catch {}
          }
          // 디버그: 콘솔에 이동 로그
          try { console.debug("[TabRescue] focus ->", next); } catch {}
          break;
        }
      }
    };
    ...
    try { console.debug("[TabRescue] mounted. path=", location.pathname); } catch {}
    return () => {
      window.removeEventListener("keydown", handler, true);
      try { console.debug("[TabRescue] unmounted"); } catch {}
    };
  }, []);
