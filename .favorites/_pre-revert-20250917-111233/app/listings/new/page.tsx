...
type NumProps = {
  ...
  placeholder?: string;
  name?: string;
  tabIndex?: number;                // Tab 순서 제어
  onKeyDown?: (e: any) => void;     // 루프용 핸들러 전달
};
...
const initForm: Form = {
  ...
  addressJibeon: "", floor: "", unit: "", availableDate: "", availableNegotiable: false,
  vacant: false, elevator: false, parking: false, pets: false,
...
export default function NewListingPage() {
  ...
  // 첫칸(보증금)에서 Shift+Tab → 막칸(메모)로
  const onFirstShiftTab = (e: any) => {
    if (e.key === "Tab" && e.shiftKey) { e.preventDefault(); focusByName("memo"); }
  };
  // 막칸(메모)에서 Tab → 첫칸(보증금)으로
  const onLastTab = (e: any) => {
    if (e.key === "Tab" && !e.shiftKey) { e.preventDefault(); focusByName("deposit"); }
  };
  // ============================
  ...
  // 제목 오른쪽에 extra(코드번호 박스)가 "바로" 붙도록 구성
  const Section = ({
      ...
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <Section title="금액">
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            <div className="md:col-span-2">
              ...
              <NumericInput
                name="deposit" tabIndex={1} onKeyDown={onFirstShiftTab}
                value={f.deposit} onValue={(v)=>set("deposit", v)} placeholder="예: 1000"
              />
            ...
            <div className="md:col-span-2">
              ...
              <NumericInput
                name="rent" tabIndex={2}
                value={f.rent} onValue={(v)=>set("rent", v)}
            ...
            <div className="md:col-span-1">
              <L>관리비(만원)</L>
              <NumericInput
                name="mgmt" tabIndex={3}
                value={f.mgmt} onValue={(v)=>set("mgmt", v)} placeholder="예: 7"
              />
            </div>
            <div className="md:col-span-3">
              <L>관리비 포함항목</L>
              <Input
                name="mgmtItems" tabIndex={4}
                value={f.mgmtItems} onChange={(e:any)=>set("mgmtItems", e.target.value)} placeholder="예: 수도, 인터넷, 청소비"
              />
            </div>
          </div>
        </Section>
        ...
        <Section title="면적 / 구조">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
              <L>전용면적(㎡)</L>
              <DecimalInput
                name="areaM2" tabIndex={8}
                value={f.areaM2} onValue={(v)=>set("areaM2", v)} placeholder="예: 44.2"
              />
            ...
            <div>
              <L>방(개)</L>
              <NumericInput
                name="rooms" tabIndex={9} maxLen={2}
                value={f.rooms} onValue={(v)=>set("rooms", v)} placeholder="예: 1"
              />
            </div>
            <div>
              <L>욕실(개)</L>
              <NumericInput
                name="baths" tabIndex={10} maxLen={2}
                value={f.baths} onValue={(v)=>set("baths", v)} placeholder="예: 1"
              />
            </div>
        ...
        <Section title="연락 / 메모">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                name="landlordName" tabIndex={11}
                value={f.landlordName} onChange={(e:any)=>set("landlordName", e.target.value)} placeholder="예: 홍길동 또는 관리인"
              />
            </div>
            <div>
              <L>임대인 연락처</L>
              <Input
                name="landlordPhone" tabIndex={12}
                value={f.landlordPhone} onChange={(e:any)=>set("landlordPhone", e.target.value)} placeholder="예: 010-1234-5678"
              />
            </div>
            <div>
              <L>임차인 성함 (관리인이면 '관리인'으로 적기)</L>
              <Input
                name="tenantName" tabIndex={13}
                value={f.tenantName} onChange={(e:any)=>set("tenantName", e.target.value)} placeholder="예: 김임차 또는 관리인"
              />
            </div>
            <div>
              <L>임차인 연락처</L>
              <Input
                name="tenantPhone" tabIndex={14}
                value={f.tenantPhone} onChange={(e:any)=>set("tenantPhone", e.target.value)} placeholder="예: 010-0000-0000"
              />
            </div>
            <div className="md:col-span-2">
              <L>메모</L>
              <Textarea
                name="memo" tabIndex={15} onKeyDown={onLastTab}
                value={f.memo} onChange={(e:any)=>set("memo", e.target.value)} placeholder="현장 특이사항, 협의 내용 등"
              />
            </div>
          </div>
        </Section>
        ...
        <div className="sticky bottom-0 z-10">
          <div className="bg-white/90 backdrop-blur border rounded-xl px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button type="button" className="px-3 h-10 border rounded-lg" onClick={saveDraft}>임시저장</button>
