...
export default function ListingsPage() {
  ...
  // === DB 목록 불러오기 (추가) ===
  const [dbItems, setDbItems] = useState<Listing[]>([]);
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
        ...
        if (alive && Array.isArray(arr)) setDbItems(arr as Listing[]);
      } catch (e) {
  ...
  // === DB + RAW 합친 소스 (추가) ===
  const SOURCE: Listing[] = useMemo(() => {
    const arr = [...dbItems, ...RAW];
    try {
  ...
  // 최종 필터 (RAW → SOURCE 로 변경)
  const rows = useMemo(() => {
    let r = SOURCE.filter(tabFilter);
    ...
    r = r.filter(x => {
      ...
      const passPark  = (!parkYes || x.parking === "가능"); // NOTE: parkYes 사용
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
        <div className="flex flex-wrap items-center gap-2">
          <div className="text-sm text-gray-700 mr-1">
            표시 <b>{rows.length}</b>건 / 전체 <b>{SOURCE.length}</b>건
          </div>
      ...
      <div className="border-y">
        <div className="overflow-auto">
          <table className="min-w-[1200px] w-full text-sm">
            ...
            <tbody>
              {rows.map((r, idx) => (
                <tr key={`${(r as any)._id ?? r.id ?? idx}`} className={"border-t " + (r.vacant ? "bg-pink-50" : "")}>
