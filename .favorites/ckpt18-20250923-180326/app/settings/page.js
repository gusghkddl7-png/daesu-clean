...
export default function Page() {
  ...
  return (
    <main className="wrap">
        ...
        <>
          ...
          <section className="cardbox">
            ...
            {loadingPeople ? <div className="empty">불러오는 중…</div> :
              ...
              <div className="tbody">{pending.map(u => <PendingRow key={u.id} u={u} onApprove={approveUser} onReject={rejectUserLocal} />)}</div>}
          </section>
          ...
          <section className="cardbox">
            ...
            {approved.length===0 ? <div className="empty">아직 승인된 직원이 없습니다.</div> :
              <ul className="gridlist">{approved.map(a=>(
                <li key={a.id} className="chiprow"><span className="name">{a.displayName || a.id}</span><span className="meta">{a.id}</span></li>
              ))}</ul>}
          </section>
            ...
            <div className="tbody">
              {filtered.map(u=>(
                <div key={u.id} className="row">
                  ...
                  <div>{u.status||"pending"}</div><div>{u.role||"user"}</div>
                  <div className="ops">
      ...
      {/* KEYBOARD (설명용 Stub – 다음 단계에 실제 바인딩) */}
...
function PendingRow({ u, onApprove, onReject }) {
  ...
  return (
    <div className="row" style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr auto", gap:"var(--gap)", padding:"var(--pad)", borderBottom:"1px solid var(--border)" }}>
      <div>
        <div className="b">{u.email || u.id}</div>
        <div style={{fontSize:11,color:"var(--muted)"}}>{u.createdAt ? new Date(u.createdAt).toLocaleString() : "-"}</div>
      </div>
      <input value={name} onChange={e=>setName(e.target.value)} placeholder="담당자 이름" className="search"
        onKeyDown={(e)=>{ if(e.key==="Enter"){ e.preventDefault(); if(name.trim()) onApprove(u.id, name); }}} />
...
const STYLES = `
  ...
  .chiprow{border:1px solid var(--border);border-radius:10px;padding:8px 10px;display:flex;align-items:start;justify-content:space-between;background:var(--card)}
  .chiprow .name{font-weight:700}.chiprow .meta{font-size:11px;color:var(--muted)}

  .table{max-width:1080px;margin:0 auto;border:1px solid var(--border);border-radius:var(--radius);background:var(--card);overflow:hidden}
  ...
  .thead,.row{display:grid;grid-template-columns:1fr .8fr .8fr .6fr .6fr 1fr;gap:var(--gap)}
  .thead{padding:10px;background:var(--card-contrast);border-bottom:1px solid var(--border);font-weight:800}
  .tbody{display:flex;flex-direction:column}
  .row{padding:12px;border-bottom:1px solid var(--border);text-align:left}
