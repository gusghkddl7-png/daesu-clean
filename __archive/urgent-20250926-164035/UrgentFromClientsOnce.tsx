...
function badge(msg:string){ let b=document.getElementById("urgent-bridge-badge") as HTMLElement|null; if(!b){ b=document.createElement("div"); b.id="urgent-bridge-badge"; document.body.appendChild(b); } b.textContent=msg; setTimeout(()=>{ try{b&&(b.style.opacity="0")}catch{} }, 6000); }
function parseDate(t:string):string|null{
  t=(t||"").trim(); if(!t) return null;
  let m=t.match(/(?:(\d{4})\s*년\s*)?(\d{1,2})\s*월\s*(\d{1,2})\s*일/); if(m){ const y=m[1]??String(new Date().getFullYear()); const iso=`${y}-${String(m[2]).padStart(2,"0")}-${String(m[3]).padStart(2,"0")}`; if(!isNaN(Date.parse(iso))) return iso; }
  m=t.match(/(?:^|\D)(20\d{2})(\d{2})(\d{2})(?:\D|$)/); if(m){ const iso=`${m[1]}-${m[2]}-${m[3]}`; if(!isNaN(Date.parse(iso))) return iso; }
  const norm=t.replace(/[^\d./-]/g," ").replace(/[.\/]/g,"-");
  m=norm.match(/(20\d{2})-(\d{1,2})-(\d{1,2})/); if(m){ const iso=`${m[1]}-${String(m[2]).padStart(2,"0")}-${String(m[3]).padStart(2,"0")}`; if(!isNaN(Date.parse(iso))) return iso; }
  m=norm.match(/(\d{2})-(\d{1,2})-(\d{1,2})/); if(m){ const iso=`20${m[1]}-${String(m[2]).padStart(2,"0")}-${String(m[3]).padStart(2,"0")}`; if(!isNaN(Date.parse(iso))) return iso; }
  m=norm.match(/\b(\d{1,2})-(\d{1,2})\b/); if(m){ const y=String(new Date().getFullYear()); const iso=`${y}-${String(m[1]).padStart(2,"0")}-${String(m[2]).padStart(2,"0")}`; if(!isNaN(Date.parse(iso))) return iso; }
  return null;
}
...
export default function UrgentFromClientsOnce(){
  useEffect(()=>{ let tries=0, stopped=false; badge("표 대기중…");
    const run=()=>{ if(stopped) return; tries++;
      ...
      try{ setTableClass(table.className||""); }catch{} if(thead){ try{ setThead((thead as HTMLElement).outerHTML);}catch{} } if(colg){ try{ setColgroup((colg as HTMLElement).outerHTML);}catch{} }
      ...
      const rows:any[]=[]; Array.from(tbody.querySelectorAll("tr")).forEach(tr=>{
      ...
      rows.sort((a,b)=>a.days-b.days); saveRows(rows); setInfo(`rows=${rows.length}`); badge(`스캔 ${rows.length}건`);
    };
    run(); const mo=new MutationObserver(()=>run()); mo.observe(document.body,{childList:true,subtree:true});
