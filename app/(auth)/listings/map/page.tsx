...
/** 면적 프리셋(평) */
const areaStepsP = [10,20,30,40,50]; // 마지막은 50평~
...
export default function ListingsMapPage() {
  ...
  // 범위 지정용 앵커(첫 클릭 시작점) — 같은 버튼 두 번 누르면 해제
  const [priceAnchor, setPriceAnchor] = useState<number|null>(null); // 만원
  const [rentAnchor,  setRentAnchor]  = useState<number|null>(null); // 만원
  const [areaAnchorP, setAreaAnchorP] = useState<number|null>(null); // 평
  ...
  /* forward geocode */
  async function tryForwardGeocode(addrKey: string) {
    const cached = coordsCacheRef.current[addrKey];
    if (cached) return cached;
    const candidates = [
      `/api/geocode/fwd?query=${encodeURIComponent(addrKey)}`,
      `/api/geocode?query=${encodeURIComponent(addrKey)}`,
      `/api/geocode?q=${encodeURIComponent(addrKey)}`,
    ];
    for (const u of candidates) {
      try {
        const r = await fetch(withBase(u, getClientBaseUrl()), {
          cache: "no-store",
        });
        if (!r.ok) continue;
        ...
        if (typeof lat === "number" && typeof lng === "number") {
          coordsCacheRef.current[addrKey] = { lat, lng };
          return { lat, lng };
        }
      } catch {}
    }
  ...
  function getCurrentBounds() {
    ...
    if (provider === "kakao" && kakaoMap.current && KM) {
      ...
      return {
        contains(lat: number, lng: number) {
          return b.contain(new KM.LatLng(lat, lng));
        },
      };
    }
    return null;
  ...
  const filtered = useMemo(() => {
    ...
    return all.filter((r) => {
      ...
      if (r.dealType === "월세") {
        if (typeof filters.rentMin === "number" && (r.rent ?? 0) < filters.rentMin)
          return false;
        if (typeof filters.rentMax === "number" && (r.rent ?? 0) > filters.rentMax)
          return false;
        if (typeof filters.priceMin === "number" && (r.deposit ?? 0) < filters.priceMin)
          return false;
        if (typeof filters.priceMax === "number" && (r.deposit ?? 0) > filters.priceMax)
          return false;
      } else {
      ...
      if (
        typeof filters.areaMinM2 === "number" &&
        (r.areaM2 ?? 0) < filters.areaMinM2
      )
        return false;
      if (
        typeof filters.areaMaxM2 === "number" &&
        (r.areaM2 ?? 0) > filters.areaMaxM2
      )
        return false;
  ...
  async function drawMarkers(fit: boolean) {
    ...
    markersRef.current.forEach((m) => {
      try {
        m.setMap ? m.setMap(null) : m.remove();
      } catch {}
    });
    markersRef.current = [];
    ...
    for (const [k, v] of grouped.entries()) {
      ...
      if (lat == null || lng == null) {
        const g = await tryForwardGeocode(k);
        if (g) {
          lat = g.lat;
          lng = g.lng;
        }
    ...
    if (fit && made > 0) {
      if (isOSM && osmMap.current && boundsOSM) {
        try {
          ...
          osmMap.current.fitBounds(boundsOSM.pad(0.2));
        } finally {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!alive || !mapRef.current || !(window as any).L) return;
      const L = (window as any).L;
      if (wrapRef.current) {
        wrapRef.current.style.height = "100vh";
        wrapRef.current.style.minHeight = "520px";
      }
      const map = L.map(mapRef.current, {
        center: [37.5386, 127.1247],
        zoom: 15,
        zoomControl: true,
      });
      ...
      map.on("moveend", () => {
        if (!fittingRef.current) setMapTick((v) => v + 1);
      });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const map = new KM.Map(mapRef.current, {
        center: new KM.LatLng(37.5386, 127.1247),
        level: 4,
      });
      kakaoMap.current = map;

      KM.event.addListener(map, "idle", () => {
        if (!fittingRef.current) setMapTick((v) => v + 1);
      });
    ...
    return () => {
      ...
      markersRef.current.forEach((m) => {
        try {
          (window as any).kakao?.maps && m.setMap?.(null);
        } catch {}
      });
      markersRef.current = [];
  ...
  /* 그룹/필터가 바뀔 때는 자동맞춤 1회 */
  useEffect(() => {
    drawMarkers(true);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [grouped]);

  /* 사용자가 지도를 움직였을 때는 맞춤 없이 마커만 업데이트 */
  useEffect(() => {
    if (mapTick > 0) drawMarkers(false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mapTick]);
  ...
  async function getBldgBrief(addr: string): Promise<BldgBrief | null> {
    const key = keyOfAddr(addr);
    if (!key) return null;
    if (bldgCache.current[key]) return bldgCache.current[key];
    try {
      const url = withBase(
        `/api/building/brief?addr=${encodeURIComponent(key)}`,
        getClientBaseUrl()
      );
      const r = await fetch(url, { cache: "no-store" });
      if (!r.ok) return null;
      const j = await r.json();
      const brief: BldgBrief = {
        use: j?.use ?? j?.mainUse,
        approvalYmd: j?.approvalYmd ?? j?.approval,
        floors: j?.floors ?? j?.floorInfo,
        elevators: j?.elevators,
        parking: j?.parking,
      };
      bldgCache.current[key] = brief;
      return brief;
    } catch {
      return null;
    }
  ...
  useEffect(() => {
    setBldg(null);
    if (!active) return;
    getBldgBrief(active.address).then((v) => setBldg(v));
  }, [active]);
  ...
  /* 버튼 클릭 공통 로직(토글/구간잡기/초기화) */
  function clickPriceButton(label: string) {
    const v = toMan(label);           // 만원
    const [bmin, bmax] = bucketRange(v, priceSteps);
    // 같은 버튼을 두 번 누르면 해제
    if (!priceAnchor && filters.priceMin === bmin && filters.priceMax === bmax) {
      setFilters(f => ({...f, priceMin: undefined, priceMax: undefined}));
      setPriceAnchor(null);
      return;
    }
    // 기존 범위/입력이 있더라도 이 버튼을 시작점으로 리셋
    if (priceAnchor == null) {
      setFilters(f => ({...f, priceMin: bmin, priceMax: bmax}));
      setPriceAnchor(v);
    } else {
  ...
  function clickRentButton(label: string) {
    const v = toMan(label);           // 만원
    const [bmin, bmax] = bucketRange(v, rentSteps);
    if (!rentAnchor && filters.rentMin === bmin && filters.rentMax === bmax) {
      setFilters(f => ({...f, rentMin: undefined, rentMax: undefined}));
      setRentAnchor(null);
      return;
    }
    if (rentAnchor == null) {
  ...
  function clickAreaButton(label: string) {
    ...
    if (open) {
      ...
      // 리셋 후 단일구간(최소만)
      setFilters(f=>({...f, areaMinM2: toM2(p), areaMaxM2: undefined}));
      setAreaAnchorP(null);
      return;
    }
    ...
    <div className="w-full flex" ref={wrapRef} style={{ height: "100vh" }}>
      ...
      <aside className="w-[360px] max-w-[45%] h-full bg-white border-r flex flex-col">
        <div className="px-3 py-2 border-b flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button className="px-2 py-1 border rounded" onClick={() => history.back()}>
              ←
            </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              className={
                ...
                (provider === "osm" ? "bg-indigo-600 text-white" : "bg-white")
              }
              onClick={() => {
                setProvider("osm");
                setSelectedKey("");
            ...
            <button
              className={
                "px-2 py-1 border rounded " +
                (provider === "kakao" ? "bg-amber-600 text-white" : "bg-white")
              }
              onClick={() => {
                if (getKakaoKey()) {
                  setProvider("kakao");
              ...
              disabled={!getKakaoKey()}
              title={getKakaoKey() ? "카카오(최신 건물/POI)" : "키 없음"}
            >
              카카오(최신)
            </button>
          </div>

          <div>{loading ? "불러오는 중…" : `전체 ${filtered.length}건`}</div>
          {SHOW_DEBUG && <div>선택: {selectedKey || "-"}</div>}
          {SHOW_DEBUG && (
            <div>
              표시 pins: {pins} · 지오코딩 실패: {geofail}
            </div>
          )}
          <div className="text-gray-500">
            ※ 핀(보라 숫자)을 누르면 해당 건물의 매물 리스트가 나타납니다.
          </div>
        </div>

        {!selectedKey && (
          <div className="p-4 text-sm text-gray-500">
            지도의 보라색 숫자(건물 단위)를 클릭하면 매물을 볼 수 있어요.
          </div>
        )}
          ...
          <div className="flex-1 overflow-auto">
            <div className="p-2 border-b bg-indigo-50 text-sm font-medium">
              ...
              <span className="text-gray-500">
                · {(grouped.get(selectedKey)?.count) ?? 0}건
              </span>
            </div>
            ...
            <div className="divide-y">
              {selectedItems.map((r) => (
                <button
                  key={(r as any)._id ?? r.id}
                  className={
                    "w-full text-left p-3 hover:bg-gray-50 " +
                    (active &&
                    ((active as any)._id ?? active.id) ===
                      ((r as any)._id ?? r.id)
                      ? "bg-indigo-50"
                      : "")
                  }
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="text-sm font-semibold">
                        ...
                        {r.dealType === "월세" && r.rent != null ? (
                          <> / {fmtWon(r.rent)}</>
                        ) : null}
                        <span className="text-xs text-gray-500 ml-2">{r.code}</span>
                      </div>
                      <div className="text-xs text-gray-700 truncate">
                        {r.buildingType || "-"} · {fmtNum(r.areaM2)}㎡ · {r.rooms ?? "-"}
                        룸 / {r.baths ?? "-"}욕실 · {r.tenantInfo || "-"}
                      </div>
      ...
      <section className="flex-1 relative" style={{ background: "#f8fafc" }}>
        ...
        <div className="absolute left-3 top-3 z-[600]">
          <button
            className="px-3 py-1 rounded border bg-white shadow text-sm font-medium"
            onClick={() => setFilterOpen((v) => !v)}
          >
            {filterOpen ? "상세검색 닫기" : "상세검색"}
          </button>
            ...
            <div className="mt-2 w-[360px] max-w-[90vw] bg-white border rounded shadow-lg p-3 text-sm space-y-3">
              ...
              <div>
                ...
                {(["매매", "전세", "월세"] as Deal[]).map((d) => (
                  <label key={d} className="mr-3 inline-flex items-center gap-1">
                    <input
                      type="checkbox"
                      checked={filters.deals.has(d)}
                      onChange={(e) =>
                        setFilters((f) => {
                          const s = new Set(f.deals);
                ...
                <label className="ml-4 inline-flex items-center gap-1">
                  <input
                    type="checkbox"
                    checked={filters.inMapOnly}
                    onChange={(e) =>
                      setFilters((f) => ({ ...f, inMapOnly: e.target.checked }))
                    }
                  />
                  지도 내만
                </label>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setFilters((f) => ({
                        ...f,
                        rentMin: e.target.value ? Number(e.target.value) : undefined,
                      }))
                    }
                  />
                  <span>~</span>
                  <input
                    ...
                    onChange={(e) =>
                      setFilters((f) => ({
                        ...f,
                        rentMax: e.target.value ? Number(e.target.value) : undefined,
                      }))
                    }
                  />
                </div>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setFilters((f) => ({
                        ...f,
                        areaMinM2: e.target.value
                          ? toM2(Number(e.target.value))
                          : undefined,
                      }))
                    }
                  />
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <label key={k} className="inline-flex items-center gap-1">
                      <input
                        type="radio"
                        name="floorType"
                        checked={filters.floorType === (k as any)}
                        onChange={() =>
                          setFilters((f) => ({
                            ...f,
                            floorType: k as Filters["floorType"],
                          }))
                        }
                      />
                      {t}
                  ...
                  <label className="inline-flex items-center gap-1 ml-2">
                    <input
                      type="radio"
                      name="floorType"
                      checked={!filters.floorType}
                      onChange={() =>
                        setFilters((f) => ({ ...f, floorType: undefined }))
                      }
                    />
                    전체
                  </label>
              ...
              <div>
                ...
                {(["1", "2", "3", "4+"] as const).map((k) => (
                  <label key={k} className="mr-3 inline-flex items-center gap-1">
                    <input
                      type="checkbox"
                      checked={filters.rooms.has(k)}
                      onChange={(e) =>
                        setFilters((f) => {
                          const s = new Set(f.rooms);
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4">
                  <label className="inline-flex items-center gap-1">
                    <input
                      type="checkbox"
                      checked={filters.wantParking}
                      onChange={(e) =>
                        setFilters((f) => ({ ...f, wantParking: e.target.checked }))
                      }
                    />
                    주차가능
                  </label>
                  <label className="inline-flex items-center gap-1">
                    <input
                      type="checkbox"
                      checked={filters.wantPets}
                      onChange={(e) =>
                        setFilters((f) => ({ ...f, wantPets: e.target.checked }))
                      }
                    />
                    반려동물가능
                  </label>
                  <label className="inline-flex items-center gap-1">
                    <input
                      type="checkbox"
                      checked={filters.wantGI}
                      onChange={(e) =>
                        setFilters((f) => ({ ...f, wantGI: e.target.checked }))
                      }
                    />
                    보증보험가능
                  </label>
              ...
              <div className="flex items-center gap-2 pt-1">
                <button
                  ...
                  onClick={() => {
                    setFilters({ ...DEFAULT_FILTERS, deals: new Set(filters.deals) });
                    setPriceAnchor(null); setRentAnchor(null); setAreaAnchorP(null);
                  }}
                >
                  가격/면적 초기화
                </button>
      ...
      <div
        className={
          "absolute right-0 top-0 h-full w-[520px] max-w-[90%] bg-white border-l shadow-xl transition-transform duration-300 " +
      ...
      >
        <div className="p-3 border-b flex items-center justify-between">
          ...
          <button className="px-2 py-1 border rounded" onClick={() => setActive(null)}>
            닫기
          </button>
        </div>
          ...
          <div className="h-[calc(100%-49px)] overflow-auto">
            ...
            <div className="p-4 border-b">
              <div className="text-lg font-bold">
                ...
                {active.dealType === "월세" && active.rent != null ? (
                  <> / {fmtWon(active.rent)}</>
                ) : null}
              </div>
              <div className="text-sm text-gray-700 mt-1">
                {active.address}
                {active.addressSub ? ` · ${active.addressSub}` : ""}
              </div>
              <div className="text-xs text-gray-500 mt-1">
                코드 {active.code} · {active.buildingType || "-"} · {fmtNum(active.areaM2)}
                ㎡ · {active.rooms ?? "-"}룸 / {active.baths ?? "-"}욕실
              </div>
            </div>
            ...
            <div className="p-4">
              <table className="w-full text-sm">
                <tbody className="[&_tr]:border-b [&_td]:py-2">
                  <tr>
                    <td className="w-36 text-gray-500">건축물 용도</td>
                    <td>{active.buildingType || "-"}</td>
                  </tr>
                  <tr>
                    <td className="text-gray-500">면적(㎡)</td>
                    <td>{fmtNum(active.areaM2)}</td>
                  </tr>
                  <tr>
                    <td className="text-gray-500">엘리베이터</td>
                    <td>{active.elevator ?? "-"}</td>
                  </tr>
                  <tr>
                    <td className="text-gray-500">주차</td>
                    <td>{active.parking ?? "-"}</td>
                  </tr>
                  <tr>
                    <td className="text-gray-500">반려동물</td>
                    <td>{active.pets ?? "-"}</td>
                  </tr>
                  <tr>
                    <td className="text-gray-500">관리비(만원)</td>
                    <td>{fmtWon(active.mgmt)}</td>
                  </tr>
                  <tr>
                    <td className="text-gray-500">임차인 정보</td>
                    <td>{active.tenantInfo || "-"}</td>
                  </tr>
                  <tr>
                    <td className="text-gray-500">연락처</td>
                    <td>{active.contact1 || active.contact2 || "-"}</td>
                  </tr>
                  <tr>
                    <td className="text-gray-500">사업자 여부</td>
                    <td>{active.isBiz ?? "-"}</td>
                  </tr>
                </tbody>
              </table>
            ...
            <div className="p-4 border-t">
              ...
              {!bldg ? (
                <div className="text-sm text-gray-500">
                  조회 중… (키가 없거나 응답이 없으면 자동 생략)
                </div>
              ) : (
                <table className="w-full text-sm">
                  <tbody className="[&_tr]:border-b [&_td]:py-2">
                    <tr>
                      <td className="w-36 text-gray-500">주용도</td>
                      <td>{bldg.use || "-"}</td>
                    </tr>
                    <tr>
                      <td className="text-gray-500">사용승인일</td>
                      <td>{bldg.approvalYmd || "-"}</td>
                    </tr>
                    <tr>
                      <td className="text-gray-500">층수</td>
                      <td>{bldg.floors || "-"}</td>
                    </tr>
                    <tr>
                      <td className="text-gray-500">승강기</td>
                      <td>{bldg.elevators || "-"}</td>
                    </tr>
                    <tr>
                      <td className="text-gray-500">주차</td>
                      <td>{bldg.parking || "-"}</td>
