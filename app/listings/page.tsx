...
type Listing = {
  ...
  vacant: boolean;
  completed: boolean;
  labelColor?: string;                       // ★ 연한색 라벨(선택)
  loft?: boolean;                            // ★ 복층
  illegal?: boolean;                         // ★ 위반건축물
  photos?: { name: string; dataUrl: string }[]; // ★ 사진(base64 dataURL)
};
...
// 비고: 1줄 + 1줄(작게)
function TwoLineCell({ text, firstLen = 20, secondLen = 20 }: { text?: string; firstLen?: number; secondLen?: number; }) {
  const t = (text ?? "").trim();
...
const catOf = (bt?: string): BtCat | "기타" => {
  ...
  return BT_CATS.find((c) => c.match.some((m) => s.includes(m.toLowerCase())))?.label ?? "기타";
};
...
export default function ListingsPage() {
  ...
  const tabFilter = (x: Listing) => {
    switch (tab) {
      ...
      case "상가/사무실매물장": return catOf(x.buildingType) === "상가/사무실";
      case "재개발매물장": return catOf(x.buildingType) === "재개발/재건축";
      default: return true;
    }
  };
  ...
  const rows = useMemo(() => {
    ...
    r = r.filter((x) => {
      ...
      return passDep && passRent && passArea && passRooms && passBaths && passElev && passPark && passPets && passBT;
    });

    return r;
  }, [items, tab, q, onlyVacant, hideCompleted, depMin, depMax, rentMin, rentMax, areaMin, areaMax, roomsMin, roomsMax, bathsMin, bathsMax, elevYes, parkYes, petsYes, btSel]);

  const resetAdvanced = () => {
    setDepMin(""); setDepMax(""); setRentMin(""); setRentMax("");
    setAreaMin(""); setAreaMax(""); setRoomsMin(""); setRoomsMax("");
    setBathsMin(""); setBathsMax(""); setElevYes(false); setParkYes(false); setPetsYes(false);
    setBtSel([]);
  };
    ...
    <main className="w-full max-w-none px-2 md:px-4 py-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
        <div className="flex flex-wrap items-center gap-2">
          <div className="text-sm text-gray-700 mr-1">표시 <b>{rows.length}</b>건 / 전체 <b>{items.length}</b>건</div>
        ...
        <div className="flex items-center gap-6">
          <label className="inline-flex items-center gap-2 select-none">
            <input type="checkbox" className="w-4 h-4" checked={onlyVacant} onChange={(e) => setOnlyVacant(e.target.checked)} /><span>공실만</span>
          </label>
          <label className="inline-flex items-center gap-2 select-none">
            <input type="checkbox" className="w-4 h-4" checked={hideCompleted} onChange={(e) => setHideCompleted(e.target.checked)} /><span>거래완료 숨기기(검색시 표시)</span>
          </label>
        </div>
      </div>
              ...
              {rows.map((r, idx) => {
                ...
                return (
                  <tr
                    ...
                    style={r.labelColor ? { background: r.labelColor } : undefined}
                  >
                    <td className="px-2 py-2">{hasPhoto ? <span title="사진 있음">📷</span> : null}</td>
                    ...
                    <td className="px-3 py-2 truncate">{`${fmtWon(r.deposit)} / ${fmtWon(r.rent)} / ${fmtWon(r.mgmt)}`}</td>
                    <td className="px-3 py-2"><TenantCell info={r.tenantInfo} /></td>
                    <td className="px-3 py-2">
                      <div className="truncate">{r.address}</div>
                      {r.addressSub && <div className="text-xs text-gray-500 truncate">{r.addressSub}</div>}
                    </td>
                    <td className="px-3 py-2">
                    ...
                    <td className="px-3 py-2 truncate">{(r.rooms ?? 0)} / {(r.baths ?? 0)}</td>
                    ...
                    <td className="px-3 py-2">
                      ...
                      {r.contact2 ? <div className="text-xs truncate">{fmtPhone(r.contact2)}</div> : null}
                    </td>
                    <td className="px-3 py-2 truncate">{r.isBiz ?? "-"}</td>
                    <td className="px-3 py-2 align-top"><TwoLineCell text={r.memo} firstLen={20} secondLen={20} /></td>
                  </tr>
                );
              })}
              {rows.length === 0 && (
                <tr>
                  <td colSpan={17} className="px-3 py-10 text-center text-gray-500">조건에 맞는 매물이 없습니다.</td>
                </tr>
              )}
            </tbody>
          </table>
            ...
            <div className="p-5 overflow-auto space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input value={depMin} onChange={(e: any) => setDepMin(e.target.value.replace(/\D/g, ""))} placeholder="최소" className="border rounded px-2 h-9 text-sm w-full"/>
                      <span className="text-gray-400">~</span>
                      <input value={depMax} onChange={(e: any) => setDepMax(e.target.value.replace(/\D/g, ""))} placeholder="최대" className="border rounded px-2 h-9 text-sm w-full"/>
                    </div>
                  </div>
                  <div>
                    <L>월세(만원)</L>
                    <div className="flex items-center gap-2">
                      <input value={rentMin} onChange={(e: any) => setRentMin(e.target.value.replace(/\D/g, ""))} placeholder="최소" className="border rounded px-2 h-9 text-sm w-full"/>
                      <span className="text-gray-400">~</span>
                      <input value={rentMax} onChange={(e: any) => setRentMax(e.target.value.replace(/\D/g, ""))} placeholder="최대" className="border rounded px-2 h-9 text-sm w-full"/>
                    </div>
                  </div>
                  <div>
                    <L>면적(㎡)</L>
                    <div className="flex items-center gap-2">
                      <input value={areaMin} onChange={(e: any) => setAreaMin(e.target.value.replace(/\D/g, ""))} placeholder="최소" className="border rounded px-2 h-9 text-sm w-full"/>
                      <span className="text-gray-400">~</span>
                      <input value={areaMax} onChange={(e: any) => setAreaMax(e.target.value.replace(/\D/g, ""))} placeholder="최대" className="border rounded px-2 h-9 text-sm w-full"/>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="rounded-xl border bg-gray-50 p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {BT_CATS.map((c) => {
                    ...
                    return (
                      <button key={c.label} onClick={() => setBtSel((s) => (active ? s.filter((v) => v !== c.label) : [...s, c.label]))}
              ...
              <section className="rounded-xl border bg-gray-50 p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <L>방(최소~최대)</L>
                    <div className="flex items-center gap-2">
                      <input value={roomsMin} onChange={(e: any) => setRoomsMin(e.target.value.replace(/\D/g, ""))} placeholder="최소" className="border rounded px-2 h-9 text-sm w-full"/>
                      <span className="text-gray-400">~</span>
                      <input value={roomsMax} onChange={(e: any) => setRoomsMax(e.target.value.replace(/\D/g, ""))} placeholder="최대" className="border rounded px-2 h-9 text-sm w-full"/>
                    </div>
                  </div>
                  <div>
                    <L>욕실(최소~최대)</L>
                    <div className="flex items-center gap-2">
                      <input value={bathsMin} onChange={(e: any) => setBathsMin(e.target.value.replace(/\D/g, ""))} placeholder="최소" className="border rounded px-2 h-9 text-sm w-full"/>
                      <span className="text-gray-400">~</span>
                      <input value={bathsMax} onChange={(e: any) => setBathsMax(e.target.value.replace(/\D/g, ""))} placeholder="최대" className="border rounded px-2 h-9 text-sm w-full"/>
                    </div>
                  </div>
              ...
              <section className="rounded-xl border bg-gray-50 p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <L>LH/SH</L>
                    <select value={lhsh} onChange={(e: any) => setLhsh(e.target.value)} className="border rounded px-2 h-9 text-sm w-full">
                      <option value="">전체</option><option>LH</option><option>SH</option>
                    </select>
                  </div>
                  <div>
                    <L>보증기관(HUG/HF)</L>
                    <select value={guar} onChange={(e: any) => setGuar(e.target.value)} className="border rounded px-2 h-9 text-sm w-full">
