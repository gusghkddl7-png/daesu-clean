...
export default function ListingsPage() {
  // [daesu:toolbar-effects] — 툴바 재배치/문구제거/필터 버튼 주입
  useEffect(() => {
    try {
      // 3-1) "+ 매물등록" 오른쪽 끝으로
      const addBtn = Array.from(document.querySelectorAll('button,a'))
        .find(el => /매물등록/.test((el.textContent||'').trim()));
      if (addBtn) {
        const wrapper = addBtn.parentElement;
        if (wrapper) {
          wrapper.style.marginLeft = 'auto';     // 오른쪽으로 밀기
    ...
    try {
      // 3-2) "기준 고정(체크포인트8)" 문구 제거
      const label = Array.from(document.querySelectorAll('body *'))
        .find(el => /기준\s*고정.*체크포인트\\s*\\d+/.test((el.textContent||'').replace(/\s+/g,' ')));
      if (label) label.remove();
    } catch(e) {}

    try {
      // 3-3) 검색은 왼쪽, "공실만/거래완료 숨기기"는 오른쪽 끝
      const search = document.querySelector('input[placeholder*="검색"],input[placeholder*="주소"],input[placeholder*="메모"]');
      const lblVac = Array.from(document.querySelectorAll('label')).find(el => /공실만/.test((el.textContent||'')));
      const lblHide = Array.from(document.querySelectorAll('label')).find(el => /거래완료\s*숨기기/.test((el.textContent||'')));
      if (search && lblVac) {
        const toolbar = search.closest('form,div,section,header') || search.parentElement?.parentElement;
        const boxWrap = lblVac.parentElement;
        if (toolbar && boxWrap) {
          // 체크박스 묶음을 툴바 끝으로 이동
          toolbar.appendChild(boxWrap);
          (boxWrap as HTMLElement).style.marginLeft = 'auto';
          (boxWrap as HTMLElement).style.display = 'flex';
          (boxWrap as HTMLElement).style.gap = '8px';
          // 거래완료 숨기기가 따로 떨어져 있다면 같이 붙이기
          if (lblHide && lblHide.parentElement && lblHide.parentElement !== boxWrap) {
            boxWrap.appendChild(lblHide.parentElement);
          }
        }
      }
    } catch(e) {}
    ...
    try {
      // 3-4) 검색 옆에 LH/SH, 허그/HF, 보증보험가능 버튼 주입 (+ 단일 토글식 필터)
      const search = document.querySelector('input[placeholder*="검색"],input[placeholder*="주소"],input[placeholder*="메모"]');
      if (search) {
        const holder = document.createElement('div');
        holder.className = 'inline-flex gap-2 ml-3 align-middle';
        holder.innerHTML = `
        ...
        `;
        (search.parentElement || search).insertAdjacentElement('afterend', holder);
        ...
        const apply = (key: string) => {
          ...
          if (key === 'LHSH') re = /(LH|SH)/i;
          else if (key === 'HUGHF') re = /(HUG|HF)/i;
        ...
        holder.querySelectorAll('button').forEach(btn => {
          btn.addEventListener('click', () => {
            ...
            holder.querySelectorAll('button').forEach(bb => bb.setAttribute('data-active','0'));
