      ...
      <div className="border rounded-xl overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              {cols.map((c, idx)=>(
                <th key={c.key} className="px-3 py-2 text-left relative select-none" style={{width:c.width, minWidth:c.width}}>
                  {c.label}
                  <div className="absolute top-0 right-0 h-full w-1 cursor-col-resize" onMouseDown={(e)=>startResize(e, idx)} title="드래그하여 열 너비 조절"/>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.map(r=>(
              <tr key={r.id} className={`border-b last:border-b-0 cursor-pointer hover:bg-gray-50 ${rowTint(r)}`} onClick={()=>openEdit(r)}>
                {cols.map(c=>(
                  <td key={c.key} className="px-3 py-2" style={{width:c.width, minWidth:c.width}}>
                    {c.render(r)}
                  </td>
                ))}
              </tr>
            ...
            {filtered.length===0 && (
              <tr><td className="px-3 py-8 text-center text-gray-500" colSpan={cols.length}>조건에 맞는 매물이 없습니다.</td></tr>
            )}
          </tbody>
        </table>
      </div>
